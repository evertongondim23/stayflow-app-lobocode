...
import { GuestDetailQuickActions } from "../../components/guest/GuestDetailQuickActions";
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`px-2 py-1 rounded-full text-xs font-bold border border-current/10 ${styles[status] || "bg-gray-100"}`}
    >
...
function formatGuestPanelDate(dateStr: string | undefined): string {
  if (!dateStr) return "—";
...
function formatGuestPanelBRL(value: number): string {
...
/** Referência curta para exibir no painel (evita #mock em ids tipo res_mock_2). */
function reservationShortRef(id: string): string {
  if (id.startsWith("res_mock_")) return id.replace("res_mock_", "");
  return id;
}

const FALLBACK_CONSUMPTION_LINES = [
  { description: "Frigobar — Água s/ gás", amount: 8 },
  { description: "Restaurante — Jantar", amount: 120 },
  { description: "Lavanderia", amount: 45 },
] as const;
...
export function ReceptionDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "arrivals" | "in_house" | "departures" | "rooms"
  >("arrivals");
  const [searchTerm, setSearchTerm] = useState("");
  const [filterStatus, setFilterStatus] = useState<"ALL" | "READY" | "PENDING">(
    "ALL",
  );
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setSelectedReservationId(null);
    };
  ...
  const renderArrivalsRoomPickerSheet = () => {
    ...
    const guestLabel = res ? (getGuest(res.guestId)?.name ?? "Hóspede") : "";
    return (
      <Sheet
        open={!!roomPickerOpen}
        onOpenChange={(open) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <BarChart2 size={20} className="text-sky-600" /> Ocupação
                  Semanal
                </h3>
                <div className="h-64">
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <BedDouble size={20} className="text-sky-600" /> Status dos
                  Quartos
                </h3>
  ...
  const renderGuestDetailsPanel = () => {
    ...
    if (!displayRes) {
      ...
      const mocks: any = {
        ...
        res_mock_3: {
          id: "res_mock_3",
          guestName: "Carla e Família",
          roomNumber: "304",
          checkInDate: "2023-10-25",
          checkOutDate: "2023-10-30",
          balance: 0.0,
          status: "CHECKED_IN",
          email: "carla.fam@email.com",
    ...
    const roomNo = displayRes.roomNumber ?? displayRes.room ?? "—";
    const roomMeta = getRoom(
      typeof roomNo === "string" ? roomNo : String(roomNo),
    );
    const roomTypeLabel = roomMeta?.type ?? "Standard";

    const expenses = displayRes.expenses as Reservation["expenses"] | undefined;
    const consumptionRows =
      Array.isArray(expenses) && expenses.length > 0
        ...
        : FALLBACK_CONSUMPTION_LINES.map((l) => ({
    ...
    return (
      <div className="fixed inset-0 z-50 flex justify-end">
        <button
          ...
          aria-label="Fechar painel do hóspede"
        ...
        <div
          ...
          className="relative z-10 flex h-dvh max-h-dvh w-full max-w-md min-h-0 flex-col border-l border-slate-200 bg-white shadow-2xl animate-in slide-in-from-right duration-300"
        >
          {/* Header */}
          <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 bg-slate-50 p-4 sm:p-5">
            <div className="flex min-w-0 items-center gap-3">
              <Avatar className="h-14 w-14 shrink-0 ring-4 ring-white shadow-sm sm:h-16 sm:w-16">
                <AvatarFallback className="bg-sky-600 text-lg font-bold text-white sm:text-xl">
                  {displayGuest?.name?.substring(0, 2).toUpperCase() || "GU"}
                </AvatarFallback>
              </Avatar>
              <div className="min-w-0">
                <h2
                  id="guest-panel-title"
                  className="truncate text-lg font-bold text-slate-900 sm:text-xl"
                >
                  {displayGuest?.name ?? "Hóspede"}
                </h2>
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <StatusBadge status={displayRes.status} />
                  <span className="font-mono text-xs text-slate-500">
                    #{reservationShortRef(displayRes.id)}
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Scrollable Content — min-h-0 exige encadeamento flex para o scroll funcionar */}
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain p-4 pb-3 sm:p-5 sm:pb-4">
            <div className="space-y-4">
              {/* KPIs: coluna única no drawer = menos aperto e valores legíveis */}
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <Badge
                    variant="outline"
                    className="w-fit bg-slate-50 text-xs font-medium text-slate-600"
                  >
                    {roomTypeLabel}
                  </Badge>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <p
                    className={`mt-1 text-2xl font-bold leading-tight tabular-nums ${
                      displayRes.balance > 0
                        ? "text-red-600"
                        : "text-emerald-600"
                    }`}
                  >
                    {formatGuestPanelBRL(Number(displayRes.balance ?? 0))}
                  </p>
                  <p className="mt-2 text-xs text-slate-500">
                    Itens e lançamentos abaixo. Use &quot;Ver extrato&quot; para
                    ver tudo.
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/90 p-4">
                ...
                <dl className="divide-y divide-slate-200/80 text-sm">
                  <div className="grid grid-cols-1 gap-1 py-2.5 first:pt-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-baseline sm:gap-x-4">
                    <dt className="text-slate-500">Check-in</dt>
                    <dd className="font-medium text-slate-900 sm:text-right sm:tabular-nums">
                      {formatGuestPanelDate(displayRes.checkInDate)}
                    </dd>
                  </div>
                  <div className="grid grid-cols-1 gap-1 py-2.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-baseline sm:gap-x-4">
                    <dt className="text-slate-500">Check-out previsto</dt>
                    <dd className="font-medium text-slate-900 sm:text-right sm:tabular-nums">
                      {formatGuestPanelDate(displayRes.checkOutDate)}
                    </dd>
                  </div>
                  <div className="grid grid-cols-1 gap-1 py-2.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-baseline sm:gap-x-4">
                    <dt className="text-slate-500">Ocupantes</dt>
                    <dd className="font-medium text-slate-900 sm:text-right">
                      {displayRes.guests ?? 2} adultos
                    </dd>
                  </div>
                  <div className="grid grid-cols-1 gap-1 py-2.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-baseline sm:gap-x-4">
                    <dt className="text-slate-500">E-mail</dt>
                    <dd
                      className="break-all font-medium text-slate-900 sm:text-right"
                      title={displayGuest?.email}
                    >
                      {displayGuest?.email || "—"}
                    </dd>
                  </div>
                  <div className="grid grid-cols-1 gap-1 py-2.5 last:pb-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-baseline sm:gap-x-4">
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/90 p-4">
                <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="h-11 w-full shrink-0 border-sky-200 text-sm font-semibold text-sky-700 hover:bg-sky-50 sm:h-10 sm:w-auto sm:min-w-28"
                  >
                    Ver extrato
                  </Button>
                </div>
                <ul className="divide-y divide-slate-200/80 overflow-hidden rounded-lg border border-slate-100/80 bg-white text-sm">
                  {consumptionRows.map((row, idx) => (
                    ...
                    >
                      ...
                      <span className="shrink-0 font-semibold tabular-nums text-slate-900">
                        {formatGuestPanelBRL(row.amount)}
                      </span>
                    </li>
                  ))}
                </ul>
  ...
  const renderArrivalsContent = () => {
    const readyCount = reservations.filter(
      (r) => r.status === "PRECHECKIN_DONE" && r.checkInDate <= today,
    ).length;
    const pendingCount = reservations.filter(
      (r) => r.status === "INVITED" && r.checkInDate <= today,
    ).length;
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">
              Gerencie o check-in e atribuição de quartos para hoje.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 p-1 bg-white rounded-xl shadow-sm border border-slate-200">
            ...
            >
              ...
              Prontos
              <span
                className={`px-1.5 py-0.5 rounded-full text-[10px] ${filterStatus === "READY" ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"}`}
              >
                {readyCount}
              </span>
            </button>
            ...
            >
              ...
              Pendentes
              <span
                className={`px-1.5 py-0.5 rounded-full text-[10px] ${filterStatus === "PENDING" ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"}`}
              >
                {pendingCount}
              </span>
            </button>
                  ...
                  <td colSpan={4} className="p-20 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-400 animate-in zoom-in-95 duration-300">
                      ...
                      <p className="text-sm mt-2 max-w-xs mx-auto text-slate-500">
                        Não encontramos reservas correspondentes aos filtros ou
                        termo de busca atuais.
                      </p>
                      <Button
                        variant="outline"
                    ...
                    >
                      <td className="p-5 pl-6">
                        <div className="flex items-center gap-4">
                          <Avatar
                            className={`h-12 w-12 border-2 ${isReady ? "border-sky-200" : "border-slate-100"} shadow-sm`}
                          >
                            <AvatarFallback
                              className={`${isReady ? "bg-sky-100 text-sky-700" : "bg-slate-100 text-slate-500"} font-bold text-lg`}
                            >
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <Badge
                                variant="outline"
                                className="text-[10px] px-1.5 py-0 h-auto border-slate-200 text-slate-400 font-mono"
                              >
                                #{r.id.split("_")[1] || r.id}
                              </Badge>
                        ...
                        {r.roomNumber ? (
                          <div className="flex flex-col gap-3">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-12 h-12 rounded-xl flex items-center justify-center font-mono font-bold text-lg shadow-sm border ${assignedRoom?.status === "VACANT_CLEAN" ? "bg-white border-emerald-200 text-emerald-700" : "bg-orange-50 border-orange-200 text-orange-700"}`}
                              >
                                {r.roomNumber}
                              </div>
                      ...
                      <td className="p-5 text-right pr-6">
                        {isReady ? (
                          <Button
                            ...
                            onClick={() =>
                              toast.success(
                                `Iniciando Validação para ${guest?.name}...`,
                              )
                            }
                          >
                            <CheckCircle
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex justify-between items-end mb-2">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">
              Gestão de estadias em andamento e contas.
            </p>
          </div>
                ...
                >
                  ...
                  <td className="p-5">
                    <div className="flex flex-col text-xs gap-1.5">
                      <span className="text-slate-500 flex items-center gap-2 font-medium">
                        ...
                        <span>
                          Entrada:{" "}
                          <span className="text-slate-700">{data.checkIn}</span>
                        </span>
                      </span>
                      <span className="text-slate-500 flex items-center gap-2 font-medium">
                  ...
                  <td className="p-5">
                    <div className="flex items-center gap-2">
                      <div
                        className={`p-1.5 rounded-lg ${data.balance > 0 ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-500"}`}
                      >
                        <CreditCard size={16} />
                      </div>
                      <span
                        className={`font-mono font-bold text-base ${data.balance > 0 ? "text-red-600" : "text-emerald-600"}`}
                      >
                  ...
                  <td className="p-5 text-right pr-6">
                    <div className="flex justify-end">
                      <Button
                        type="button"
                        size="sm"
                        className="h-10 min-w-[6.5rem] rounded-lg bg-sky-600 px-4 text-sm font-semibold text-white shadow-sm hover:bg-sky-700 focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2"
  ...
  const renderDeparturesContent = () => {
    ...
    const filteredDepartures =
      filterStatus === "ALL"
        ? MOCK_DEPARTURES
        : MOCK_DEPARTURES.filter(
            (d) =>
              d.status ===
              (filterStatus === "READY" ? "CHECKED_OUT" : "CHECKOUT_PENDING"),
          );
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">
              Processamento de check-outs e fechamento de contas.
            </p>
          </div>
              ...
              {filteredDepartures.map((data, idx) => (
                ...
                >
                  ...
                  <td className="p-5">
                    <div className="flex items-center gap-2">
                      <span
                        className={`font-mono font-bold text-lg ${data.balance > 0 ? "text-red-600" : "text-emerald-600"}`}
                      >
                        R$ {data.balance.toFixed(2)}
                      </span>
                    </div>
                    ...
                    {data.status !== "CHECKED_OUT" ? (
                      <div className="flex justify-end">
                        <Button
                          type="button"
                          size="sm"
                          className="h-10 min-w-[8.5rem] rounded-lg bg-orange-500 px-4 text-sm font-semibold text-white shadow-md shadow-orange-900/15 hover:bg-orange-600 focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-2"
                          onClick={() => {
                            if (data.balance > 0) {
                              toast.warning(
                                `Receba R$ ${data.balance.toFixed(2)} antes de finalizar.`,
                              );
                            } else {
                              toast.success(
                                `Check-out de ${data.guestName} realizado!`,
                              );
                            }
                          }}
                        >
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">
            Visão geral do status de limpeza e ocupação.
          </p>
        </div>
        ...
        <div className="flex gap-4 text-sm font-medium text-slate-500 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-emerald-500"></div> Limpo
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-orange-500"></div> Sujo
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-sky-500"></div> Ocupado
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-slate-400"></div> Manutenção
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
        {rooms.map((room, idx) => {
          ...
          const config =
            statusConfig[room.status] || statusConfig["VACANT_CLEAN"];
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div
                  className={`p-1.5 rounded-lg ${room.status === "OCCUPIED" ? "bg-sky-200 text-sky-700" : "bg-slate-100 text-slate-400"}`}
                >
                  <Icon size={16} />
                </div>
              </div>
  ...
  const renderRoomManagementModal = () => {
    ...
    const currentStay = reservations.find(
      ...
      >
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Sai em: {currentStay.checkOutDate}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    toast.success("Nota adicionada ao time de manutenção.")
                  }
                >
                  <Wrench size={16} />
                </Button>
          ...
          <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end">
            <Button variant="ghost" onClick={() => setSelectedRoomNumber(null)}>
              Fechar
            </Button>
          </div>
        </div>
      ...
      <aside className="w-72 bg-slate-900 text-slate-300 flex flex-col shadow-xl z-20">
        <div className="p-8 pb-4">
          <div className="space-y-3 mb-6">
            ...
            <span className="text-xs text-sky-400 font-bold uppercase tracking-widest">
              Painel recepção
            </span>
          </div>
        </div>
          ...
          {isUserMenuOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-2 bg-slate-800 rounded-xl shadow-2xl border border-slate-700 overflow-hidden animate-in slide-in-from-bottom-2 z-50">
              ...
              {STAFF_LIST.map((user) => (
                ...
                >
                  <div
                    className={`w-2.5 h-2.5 rounded-full ring-2 ring-offset-2 ring-offset-slate-800 ${currentUser === user ? "bg-sky-400 ring-sky-400" : "bg-slate-600 ring-transparent"}`}
                  ></div>
                  {user}
                </button>
              ))}
